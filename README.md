...
- **G HUB**：先在“设备按键探测器”中复制 Lua，在 G HUB 脚本日志中按住要用的物理键，记录原始事件 `arg` 和命中的状态键 `1–5`。填写“启动事件编号”和“启动状态键”后选择触发模式：单次播放会完整执行一次；长按播放在启动状态键松开后停止；切换播放在该状态键再次按下后停止。可选“独立停止状态键”会在所有模式中止当前 Lua 并释放正在按住的琴键和变调键。高编号物理键（例如 G10、G11）不能直接作为状态查询编号，必须按探测结果映射到 1–5。
...
- **口琴鼠标宏录制助手（测试版）**：下载 `.ahk` 后使用 [AutoHotkey v2](https://www.autohotkey.com/) 运行。先在目标鼠标宏软件中创建单次播放宏并启动录制，再关闭助手提示并在 5 秒倒计时内回到录制界面；助手会模拟当前曲谱的一次键盘和鼠标变调输入。它依赖目标软件捕获 AutoHotkey 的合成输入，无法保证所有软件兼容；请先录制短谱，并确保游戏窗口没有焦点。

宏绑定设置只用于 G HUB Lua 导出，不会写入谱子分享文件；Synapse XML 与 ROG GMAC 导出不受绑定键设置影响。

如果鼠标没有额外侧键，请选择“切换播放”，将独立停止状态键留空，并重复按映射后的启动状态键即可停止当前 Lua。独立停止键只负责停止当前导出的 Lua；G HUB Lua 无法从一个脚本强制关闭其他独立脚本或设备宏。默认变调占用 `1`、`2`、`3` 时，通常可将 G10、G11 在 G HUB 中映射为两个不同的未占用状态键（常见为 `4`、`5`），但必须以探测日志为准。
